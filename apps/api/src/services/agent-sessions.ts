...
import type { AutonomousAgent, AutonomousLoopEvent, AutonomousLoopRequest } from "../../../../core/orchestration/autonomous-loop"
...
function loadSessions() {
  ...
  try {
    ...
    for (const session of persisted.slice(-MAX_PERSISTED_SESSIONS)) sessions.set(session.id, { ...session, subscribers: new Set() })
  } catch {
...
function persistSessions() {
  ...
  const persisted = Array.from(sessions.values()).slice(-MAX_PERSISTED_SESSIONS).map(({ subscribers: _subscribers, ...session }) => session)
...
export function createAgentSession(request: AutonomousLoopRequest) {
  const session: AgentSession = {
    id: createId(), request, status: "queued", createdAt: new Date().toISOString(), events: [], subscribers: new Set()
  }
...
export function getAgentSession(id: string) { return sessions.get(id) }

export function listAgentSessions(limit = 25) {
  return Array.from(sessions.values()).slice(-Math.max(1, Math.min(limit, MAX_PERSISTED_SESSIONS))).reverse().map(serializeAgentSession)
}
...
async function runAgentSession(session: AgentSession) {
  ...
  session.lastTransitionAt = session.startedAt
  persistSessions()

  try {
    session.result = await house.autonomousLoop.run(session.request, (event) => {
      session.events.push(event)
      session.activeAgent = event.agent
      session.activeAction = event.action
      session.activeIteration = event.iteration
      session.lastTransitionAt = new Date().toISOString()
      persistSessions()
      for (const subscriber of session.subscribers) subscriber(event)
    })

    session.status = session.result && typeof session.result === "object" && "success" in session.result && session.result.success ? "completed" : "failed"
  } catch (error) {
    session.status = "failed"
    session.result = { success: false, error: error instanceof Error ? error.message : "Agent session failed" }
  } finally {
    session.completedAt = new Date().toISOString()
    session.lastTransitionAt = session.completedAt
    persistSessions()
  }
}
